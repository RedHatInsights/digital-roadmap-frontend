...
const LifecycleTab: React.FC<React.PropsWithChildren> = () => {
    ...
    if (
      ...
      if (Array.isArray(versions)) {
        const totalAvailable =
          Array.isArray(rhelVersionOptions) && rhelVersionOptions.length > 0
            ? rhelVersionOptions.length
            : new Set((result as SystemLifecycleChanges[]).map((d) => d.major)).size;

        if (versions.length === 0) {
          result = [];
        } else if (versions.length < totalAvailable) {
  ...
  const initializeData = async (viewFilter: string) => {
    ...
    try {
      ...
      setRhelVersionFilter(dynOptions);
  ...
  const resetFilters = () => {
    ...
    setFilterField('Name');
    setRhelVersionFilter(rhelVersionOptions);
            ...
            onFilterFieldChange={(field) => {
              ...
              const dataSource =
                lifecycleDropdownValue === RHEL_SYSTEMS_DROPDOWN_VALUE
                  ...
                  : appLifecycleChanges;
              applyAllActiveFilters(dataSource, lifecycleDropdownValue, nameFilter, field, rhelVersionFilter);
            }}
