...
export const LifecycleFilters: React.FunctionComponent<LifecycleFiltersProps> = ({
  ...
  const [selectedRhelVersions, setSelectedRhelVersions] = React.useState<string[]>([...rhelVersionOptions]);
  ...
  React.useEffect(() => {
    setSelectedRhelVersions((prev) => {
      ...
      const next = intersection.length > 0 ? intersection : [...rhelVersionOptions];
  ...
  const handleClearAllFilters = () => {
    setNameFilter('');
    const all = [...rhelVersionOptions];
    setSelectedRhelVersions(all);
    onFilterFieldChange?.('Name');
    onRhelVersionsChange?.(all);
  };
  ...
  return (
    <div className="drf-lifecycle__filters">
      <Toolbar
        className="drf-lifecycle__filters-toolbar"
        clearFiltersButtonText={
          lifecycleDropdownValue === 'Red Hat Enterprise Linux' ? 'Reset filters' : 'Clear all filters'
        }
        ...
        <ToolbarContent className="drf-lifecycle__filters-toolbar-group">
          <ToolbarGroup variant="filter-group">
            ...
            <ToolbarFilter
              ...
              chips={isSystemsView ? selectedRhelVersions.map((v) => ({ key: v, node: v })) : undefined}
